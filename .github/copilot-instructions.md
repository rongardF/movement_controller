...
- `.github/skills/ros2-python-interface-package/` — Build a single `ament_cmake` package that ships both Python code and generated ROS 2 interfaces (`.msg`/`.srv`/`.action`). Use when a package needs both `ament_python_install_package` and `rosidl_generate_interfaces`, or when a build fails with `ament_cmake_python_symlink_<pkg>` / `..._egg` "target already exists".
